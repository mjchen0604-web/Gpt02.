...
func HasEnabledChannelTypeForModel(group string, modelName string, channelType int) bool {
	if group == "" || modelName == "" {
		return false
	}
	var count int64
...
func HasEnabledChatCoreForModel(group string, modelName string) bool {
	return HasEnabledChannelTypeForModel(group, modelName, constant.ChannelTypeChatCore)
}
...
func UpdateAbilityByTag(tag string, newTag *string, weight *uint) error {
	ability := Ability{}
	if newTag != nil {
		ability.Tag = newTag
	}

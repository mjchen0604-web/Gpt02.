...
`ChatCore` 用于把 `new-api` 当作管理层，把 `chat` / `ChatMock` 当作真正的模型执行层。

分工如下：
- `new-api` 负责用户、令牌、分组、额度、计费、管理后台
- `chat` 负责账号轮询、`chatgpt-backend`、`codex-app-server`、`service_tier`、模型映射和真实上游调用

## 适用场景

适合下面这种结构：
- 你已经有一套可用的 `chat` 服务
- 你想保留 `chat` 的账号池和 Codex 能力
- 你想把用户、密钥、分组、配额、计费、管理后台交给 `new-api`
...
`chat` 需要先能独立工作，至少满足：
- OpenAI 兼容入口可用
- `chatgpt-backend` 或 `codex-app-server` 已配置
...
- 调用这些模型时，不会再因为 `model_price_error` 被 `new-api` 提前拦截
...
- `new-api` 只保留管理功能
- 所有执行类渠道统一用 `ChatCore`
- 不再让 `new-api` 直接连 OpenAI / Claude / Gemini 等 provider

这样结构最清楚，也最符合“`new-api` 做管理，`chat` 做功能”的目标。

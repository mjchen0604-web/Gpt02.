...
def get_available_upstream_candidate_count() -> int:
    ...
    *,
    ...
    if manager is not None and hasattr(manager, "get_request_candidates"):
        try:
            ...
            candidates = []
    if not candidates and manager is None:
    ...
    if not candidates and manager is not None:
        return None, build_openai_error_response(
            build_error_info(
                ...
                raw_status=429,
                raw_message="No eligible Codex candidates are currently available",
                raw_body={"message": "No eligible Codex candidates are currently available"},
            )
        )
